...
Proof.
  intros X. induction l1 as [| x1 l1'].
  - simpl. reflexivity.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => ble_nat 8 n)  (filter evenb l).
...
Proof.
  intros X Y f. induction l as [| n l'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'.
    assert (forall {X: Type} (n: X) (l: list X), snoc l n = l ++ [n]).
    + induction l as [| x l1']. simpl. reflexivity. simpl. rewrite IHl1'. reflexivity.
    + rewrite H. rewrite H. rewrite <- H.
      assert (forall {X Y:Type} (f: X->Y) (n: X) (l: list X), map f (snoc l n) = map f l ++ [f n]).
      * induction l as [| x l1'].
        simpl. reflexivity.
...
Eval compute in fold_map oddb [1;2;3;4;54;6;57;457;45;23;5123].
(** [] *)
...
*)
Theorem len_ind: forall X n l, length l = n -> @index X n l = None.
Proof.
  intros. generalize dependent n. induction l as [| n' l']. simpl. reflexivity.
  intros. simpl in H. rewrite <- H. simpl. apply IHl'. reflexivity.
Qed. 
(** [] *)
...
Print one.
Eval compute in one bool negb true.

Definition plus (n m : nat) : nat :=
(*  fun nat f x => (n nat succ m). *) admit.
